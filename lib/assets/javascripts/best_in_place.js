...
BestInPlaceEditor.prototype = {
    ...
    confirmUpdate : function() {
        var confirm_text = this.element.data('bipConfirmation');
        if(confirm_text !== undefined)
        {
          if(!confirm(confirm_text))
          { // Avoid request if confirmation false
    ...
    update: function () {
        ...
        switch (this.formType) {
            case "select":
                ...
                // Handle the case where empty string should match 'nil' key in collection
                var displayValue = value;
                if (value === '') {
                    displayValue = 'nil';
                }
                $.each(this.values, function(index, arr){ 
                    if (String(arr[0]) === String(value) || (value === '' && String(arr[0]) === 'nil')) {
                        editor.element.html(arr[1]); 
                    }
                });
                break;
    ...
    initOptions: function () {
        ...
        if (self.formType === "select" || self.formType === "checkbox") {
            ...
            if (self.element.data("bipValue") != null) {
              self.collectionValue = self.element.data("bipValue");
            } else {
...
BestInPlaceEditor.forms = {
    ...
    "select": {
        ...
        getValue: function () {
            ...
            // Convert string 'nil' back to empty string to handle Ruby nil keys
            if (value === 'nil') {
                value = '';
            }
